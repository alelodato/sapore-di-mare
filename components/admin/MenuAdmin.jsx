...
import { Plus, Pencil, Trash2, Eye, EyeOff, X, Check, Upload, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function MenuAdmin({ initialSections, initialItems }) {
  ...
  const saveSection = async (form) => {
    ...
    if (editSection?.id) {
      ...
      const { data } = await supabase.from('menu_sections').insert([{ ...form, position: sections.length }]).select().single();
  ...
  const saveItem = async (form) => {
    ...
    if (editItem?.id) {
      ...
      const { data } = await supabase.from('menu_items').insert([{
        ...form, section_id: activeSection, position: sectionItems.length
      }]).select().single();
    ...
    <div>
      ...
      <div className="grid grid-cols-[280px,1fr] gap-6">
        ...
        <div className="space-y-1">
          {sections.map(s => (
            <div key={s.id}
              className={clsx('flex items-center gap-2 px-4 py-3 border cursor-pointer transition-colors group',
                activeSection === s.id
            ...
            >
              <span className="flex-1 font-mono-label text-[10px] tracking-widest uppercase truncate">{s.name}</span>
              <span className={clsx('w-1.5 h-1.5 rounded-full shrink-0', s.active ? 'bg-green-500' : 'bg-red-500/50')} />
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={e => { e.stopPropagation(); toggleSection(s.id, s.active); }}
                  className="p-1 hover:text-gold transition-colors"
                >
                  {s.active ? <EyeOff size={11} /> : <Eye size={11} />}
                </button>
                <button onClick={e => { e.stopPropagation(); setEditSection(s); }}
                  className="p-1 hover:text-gold transition-colors"
                >
                  <Pencil size={11} />
                </button>
        ...
        <div>
          ...
          <div className="space-y-3">
            {sectionItems.map(item => (
              <div key={item.id}
                className={clsx('flex items-start gap-4 p-5 border transition-colors',
            ...
            {sectionItems.length === 0 && activeSection && (
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.price} onChange={set('price')} placeholder="£24"
                className="w-full bg-noir border border-white/10 text-cream text-sm px-4 py-3 focus:outline-none focus:border-gold/50" />
            </div>
            <div>
              <label className="block section-label text-[10px] mb-2">Note</label>
              <input value={form.note} onChange={set('note')} placeholder="e.g. For two"
                className="w-full bg-noir border border-white/10 text-cream text-sm px-4 py-3 focus:outline-none focus:border-gold/50" />
            </div>
          </div>
          {/* Image upload */}
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="active" className="font-mono-label text-[10px] text-cream/50 tracking-widest uppercase">Active (visible on menu)</label>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-noir/90 backdrop-blur-sm">
      <div className="bg-noir-mid border border-white/10 w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-white/5">
          <h2 className="font-display text-2xl font-light text-cream">{section ? 'Edit Section' : 'New Section'}</h2>
          <button onClick={onClose} className="text-cream/40 hover:text-gold transition-colors"><X size={18} /></button>
        </div>
        <div className="p-6 space-y-5">
          <div>
            <label className="block section-label text-[10px] mb-2">Name *</label>
          ...
          <div>
            <label className="block section-label text-[10px] mb-2">Slug * (es. raw-fish)</label>
            <input value={form.slug} onChange={set('slug')} placeholder="raw-fish"
              className="w-full bg-noir border border-white/10 text-cream text-sm px-4 py-3 focus:outline-none focus:border-gold/50" />
          </div>
          <div>
            <label className="block section-label text-[10px] mb-2">Subtitle</label>
            <input value={form.subtitle} onChange={set('subtitle')} placeholder="Crudo"
              className="w-full bg-noir border border-white/10 text-cream text-sm px-4 py-3 focus:outline-none focus:border-gold/50" />
          </div>
          <div className="flex items-center gap-3">
            <input type="checkbox" id="sec-active" checked={form.active} onChange={e => setForm(p => ({ ...p, active: e.target.checked }))} className="accent-gold" />
            <label htmlFor="sec-active" className="font-mono-label text-[10px] text-cream/50 tracking-widest uppercase">Active</label>
          </div>
          <button onClick={() => onSave(form)} disabled={loading}
            className="w-full btn-gold-filled py-3 font-mono-label text-xs tracking-widest uppercase disabled:opacity-50"
          >
